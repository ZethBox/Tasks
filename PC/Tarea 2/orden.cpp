/*
* Elaborado por: Sebastian Molano
* El codigo parece no funcionar en algunas versiones de Dev-C++ ya que no parece hacer * * una referencia correcta.
* Probado en Repl.it
*/
...
/**
* Genera un n�mero aleatorio en un rango dado
* Entradas:
...
string generarAleatorios(int n) {
  int* contenidos = new int[n];
  ...
  for(int i = 0; i < n; i++) {
    aleatorio = generarAleatorio(n + 1);
    if(!contains(n, contenidos, aleatorio)) {
      *(contenidos + i) = aleatorio;
    }
    else {
      i--;
    }
